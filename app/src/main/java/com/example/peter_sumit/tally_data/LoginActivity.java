...
//*****************************************************************************
    ...
    void readData(){
        boolean b=false;
        try {
            DataInputStream din = new DataInputStream(openFileInput(LoginActivity.compName + ".txt"));
            b=true;
            int len=din.readInt();
    ...
    class GetData extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... params) {         //DOWNLOADING DATA FROM HOSTINGER
            try {
                ...
                DataOutputStream dos = new DataOutputStream(openFileOutput(LoginActivity.compName+".txt",0));
                dos.writeInt(len);
                for(int i=0;i<len;i++){
        ...
        protected void onPostExecute(String s) {
            dialog.dismiss();
            if(b) {
                startActivity(new Intent(cont, HomeScreen.class));
                finish();
            }else{
